...
fn exec_no_simd(read: &Vec<u8>, reference: &Vec<u8>) -> i32{
...
unsafe fn exec_simd(read: &Vec<u8>, reference: &Vec<u8>) -> i32{
    let mut m: Vec<Vec<i32>> = vec![vec![0; read.len()]; reference.len()];
    for i in 0..reference.len() {
        m[i][0] = i as i32
    }
    ...
    for i in 1..reference.len(){
        for j in (1..read.len()).step_by(4){
            let us = _mm_add_epi32(_mm_load_epi32(m[i-1].get_unchecked(j)), one_simd);
            println!("{:?}", us);
            ...
            let neq_ds = _mm_add_epi32(_mm_load_epi32(m[i-1].get_unchecked(j-1)), one_simd);
            let ds = _mm_or_epi32(_mm_and_si128(eq_diff_mask, eq_ds), _mm_andnot_si128(eq_diff_mask, neq_ds));
        }
    }
    m[reference.len()-1][read.len()-1]
...
unsafe fn set_simd_read_slice(read: &Vec<u8>, j: usize) -> __m128i {
    ...
    let simd_read_slice = transmute::<[i32; 4], __m128i,>(read_as_i32[0..4].try_into().expect("err"));
    simd_read_slice
}
pub unsafe fn prova(){
