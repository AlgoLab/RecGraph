...
fn get_path_max_chain(
    ...
    max_chain.iter().for_each(|m| {
        max_chain_seed[m.seed_id] = Some(m.to_owned());
    });
    max_chain_seed
}

// FIXME: understand how to change from chain score to heurstic distance
fn rec_chain_update(
    chains: &Vec<Vec<Option<Match>>>,
    match_len: usize,
    rec_cost: usize,
    seeds_number: usize,
    query_len: usize,
    ...
    let mut best_paths = vec![0; chains[0].len()];
        ...
        for i in 0..rec_chains.len() {
            ...
            if let Some(m) = &chains[i][j] {
                let score = rec_chains[i][j-1].score + m.dist as usize;
                let score_rec = rec_chains[best_path][j-1].score + m.dist as usize + rec_cost;
                if score < score_rec {
                    rec_chains[i][j] = Link::init(rec_chains[i][j-1].len + 1, j, score);
                } else {
                    rec_chains[i][j] = Link::init(rec_chains[best_path][j-1].len + 1, best_path, score_rec);
                }
            } else {
                let score = rec_chains[i][j-1].score + 2;
                let score_rec = rec_chains[best_path][j-1].score + rec_cost + 2;
                if score < score_rec {
                    rec_chains[i][j] =Link::init(rec_chains[i][j-1].len + 1, j, score);
                } else {
                    rec_chains[i][j] =Link::init(rec_chains[best_path][j-1].len + 1, best_path, score_rec);
            ...
            if curr_best.is_none() || rec_chains[i][j].score <= rec_chains[curr_best.unwrap() as usize][j].score {
                curr_best = Some(i);
            }
    ...
    let mut chains_score: Vec<Vec<_>> = rec_chains
        .iter()
        .map(|chain| chain.iter().map(|link| link.score).collect::<Vec<_>>())
        .collect();
