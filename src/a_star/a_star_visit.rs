        ...
        let (m_x, ins, del) = if !path_graph.nwp_rev[current_node.node]{
            ...
            path_graph.pred_hash_rev.get_preds_and_paths(current_node.node).iter().for_each(|(succ, paths)|{
                if paths[current_node.path] {
                    ...
                    (m_x, ins, del) = get_neighbours(&current_node, &crumbs, match_mis, *succ);
                }
            }); 
            (m_x, ins, del)  
        };
...
fn get_neighbours(current_node: &AStarNode, crumbs: &Vec<Vec<usize>>, match_mis: usize, succ: usize) -> (AStarNode, AStarNode, AStarNode){
    let (m_x, ins, del) = {
...
fn update_open_set(
    ...
    if let Some(old_node) = alignment_graph.remove(&(new_node.node, new_node.pos, new_node.path)) {
        if new_node.g < old_node.g {
            ...
            alignment_graph.insert((new_node.node, new_node.pos, new_node.path), new_node.clone());
        } else {
