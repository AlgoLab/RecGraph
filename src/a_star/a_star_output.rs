...
pub fn build_gaf(alignment_graph: &mut HashMap<(usize, usize, usize), AStarNode>, end_pos: &AStarNode, path_graph: &PathGraph, query: &BString) {
    ...
    cigar.reverse();
    println!("{:?}\tbest path: {}",build_cigar::build_cigar(&cigar), align.path)
}