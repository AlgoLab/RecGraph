...
pub fn exec(
    ...
    for i in 0..lnz.len() - 1 {
        ...
        let (left, right) = set_ampl_for_row_v2(i, p_arr, r_values[i], &best_scoring_pos, &ampl_for_row, sequence.len());
        ampl_for_row[i] = (left, right);
        m[i] = vec![0; right-left];
        x[i] = vec![0; right-left];
        y[i] = vec![0; right-left];
        path[i] = vec![('O', 0); right-left];
        ...
        println!("l:{} r:{} best_col: {}", left, right, best_val_pos);
    }
...
fn set_ampl_for_row_v2(i: usize, p_arr: &[usize], r_val: usize, best_scoring_pos: &[usize],ampl_for_row: &Vec<(usize, usize)>, seq_len: usize) -> (usize, usize){
    ...
    let band_end;
    if seq_len > r_val {
        band_start = cmp::max(0, cmp::min(ms, seq_len -r_val));
        band_end = cmp::min(lnz_len, cmp::max(me, seq_len-r_val));
    } else {
        band_start = 0;
        band_end = seq_len;
    }
