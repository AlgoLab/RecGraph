...
pub fn ab_ed_km(s1: &Vec<char>, s2: &Vec<char>, ampl: i32) {
    let s1_len = s1.len();
    let s2_len = s2.len();
    ...
    for i in 0..s1_len {
        for j in 0..ampl as usize {
            ...
            match (i, j) {
                ...
                _ if i == 0 => {
                    if i_32 + j_32 != ampl / 2 { // prima riga tabella
                        a[0][j] = a[0][j - 1] + 1;
                        path[0][j] = 'L';
                    } else {    // origine
                        a[i][j] = 0;
                ...
                _ if j == ampl as usize - 1 => { //bordo sup banda
                    let mut to_add = 0;
                    if s1[i] != s2[i + j - (ampl / 2) as usize] {
                        to_add = 1
                    }
    ...
    match ampl_is_enough_iterative(&path, s2_len - 1 + (ampl / 2) as usize - (s1_len - 1)) {
        true => {
            println!("EDIT DISTANCE: {}", a[s1_len - 1][s2_len - 1 + (ampl / 2) as usize - (s1_len- 1)]);
        }
        false => {
            ab_ed_km(s1, s2, ampl * 2 + 1)
        },
    }
}

fn ampl_is_enough_iterative(path: &Vec<Vec<char>>, start_col: usize) -> bool{
